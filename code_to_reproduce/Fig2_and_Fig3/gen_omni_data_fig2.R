...
# The purpose of this script is to generate supporting data for the simulations 
# which produced Figures 2 and 3 in the submitted manuscript.
# We are interested in knowing the power of different set-based tests (GBJ, GHC, SKAT, minP, OMNI)
# across different sparsity settings and under different correlation structures.

# This script is almost identical to gen_sim_data_fig2.R, but instead of just generating p-values
# we are estimating the correlation structure of the p-values for the omnibus test (necessary to calculate
# the power of the omnibus test later).
# We set all effect sizes to 0 and stop the run after the correlation matrix has
# been estimated. 

# The correlation matrix is read by plot_fig2_part1.R-plot_fig2_part4.R and is needed
# to find the power of the omnibus test statistic.
# On some clusters it may appear that the job has 'failed' because we use a stop() command to
# manually exit the job after recording the correlation matrix.
# Each job should produce one file with a name like 'd100_rho1_3_rho2_3_rho3_3_ncausal4.txt'
# where the numbers after each parameter will change depending on the specific set parameters
# we have chosen.
# The set parameters are controlled by the aID and Snum variables, which are parameters passed
# in on the command line. Our job submission scripts (*.sbatch and *.bsub) control these parameters
# so that all necessary files are generated.

# After all runs from this script as well as gen_sim_data_fig2.R have been completed, use 
# plot_fig2_part1.R-plot_fig2_part4.R to reproduce Figures 2 and 3 of the submitted manuscript.
...
#####################################################################
# Determine some parameters for this specific run.
# If finding the omnibus null distribution, this value needs to be TRUE.
OMNI_RUN <- TRUE
...
set.seed(03082017 + Snum*20 + aID + 1000)
...
# Effect sizes
if (param_row == 1) {
	beta_possible <- c(0.12, 0.10, 0.09, 0.09, 0.09, 0.09, 0.09, 0.09, 0.09, 0.09)
} else if (param_row == 2) {
	beta_possible <- c(0.11, 0.08, 0.06, 0.05, 0.04, 0.04, 0.035, 0.03, 0.03, 0.03)
} else if (param_row == 6) {
        beta_possible <- c(0.11, 0.09, 0.06, 0.05, 0.05, 0.04, 0.03, 0.03, 0.03, 0.03)
} else if (param_row == 8) {
        beta_possible <- c(0.1, 0.07, 0.05, 0.04, 0.035, 0.03, 0.025, 0.025, 0.025, 0.025)
}
beta_causal <- beta_possible[ncausal]
...
########################################################################################
# How are we generating the genotypes in this run?
if (HAPGEN_FLAG) {
...
########################################################################################
# Loop through simulations for this setting.
cat('ncausal: ', ncausal, '\n')
...
{				
	...
	if (OMNI_RUN == TRUE)
	{
		...
		# Just write it and stop (may throw an error depending on your cluster)
...
out_name <- paste('power_results_S', Snum, '_', aID, '.txt', sep='')
