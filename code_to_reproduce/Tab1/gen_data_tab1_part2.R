...
# across regions of chromosome 5 that correspond to real gene sizes.

# We generate SNPs at random regions on along chromosome 5 using HAPGEN2, generate an outcome from under the null, and apply
# GBJ and OMNI to the simulated data.

# Each job should produce one file with a name like 'chr5_typeIerr_S1_aID1.txt'
...
# After all runs from this script as well as gen_data_tab1_part1.R and gen_data_part3.R have been completed, use 
...
########################################################################################
# Read input arguments (provided automatically by job submission script)
args <- commandArgs(trailingOnly=TRUE)
aID <- as.numeric(args[1])
Snum <- as.numeric(args[2])
set.seed(05112017 + Snum*1000 + aID)
options(scipen=9)
...
# Find the start and end rows (in the legend) for this gene
glist_18 <- fread('glist-hg18.txt')
start_bp <- glist_18$V2[aID]
end_bp <- glist_18$V3[aID]
start_row <- which(leg_file$position >= start_bp)[1]
end_row <- which(leg_file$position <= end_bp)[length(which(leg_file$position <= end_bp))]
########################################################################################
		...
		if (length(bad_snps) > 0)
		{
		 ...
		 # Enough snps generated
       	if (nrow(combined_haps) >=  gene_size) {
       		...
       		# gene_size = -1 signifies keep any gene size
       		if (gene_size > 0) {
...
pow_results <- matrix(data=NA, nrow=genes_per_run*sims_per_gene, ncol=22)
pow_results <- data.frame(pow_results)
colnames(pow_results) <- c('start_row', 'end_row', 'd', 'ncausal', 'beta_causal', 'median_rho2',
...
for (i in 1:genes_per_run) {
	...
	# Interval too small? Just quit
	if (length(snps_to_keep) < 3) {
		pow_results$start_row[1] <- start_row
		pow_results$end_row[1] <- end_row
		pow_results$d[1] <- length(snps_to_keep)
		pow_results <- as.matrix(pow_results[1, ])
		break
	}
	
	# Interval large enough, keep going
	...
	for (j in 1:hapgen_per_gene) {
		...
		for (k in 1:runs_per_hapgen) {
			...
			mu <- as.matrix(G_mat[, 1:num_causal]) %*% beta_vec 
			...
			rho2_mat <- cor_mat[1:num_causal, (num_causal+1):ncol(cor_mat)]
			...
			pow_results$d[record_row] <- ncol(G_mat)
		}
	}
}
########################################################################################
...
out_name <- paste('chr5_typeIerr_S', Snum, '_', aID, '.txt', sep='')
